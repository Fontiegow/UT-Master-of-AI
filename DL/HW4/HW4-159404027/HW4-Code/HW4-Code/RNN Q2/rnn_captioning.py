...
def rnn_step_backward(dnext_h, cache):
    ...
    # ۱. عبور مشتق از تابع tanh
    # d_tanh = dnext_h * (1 - next_h^2)
    dtanh = dnext_h * (1 - next_h * next_h) # (N, H)
    
    # ۲. محاسبه مشتق نسبت به وزن‌ها و ورودی‌ها
    dx = dtanh.mm(Wx.t())        # (N, D)
    dprev_h = dtanh.mm(Wh.t())   # (N, H)
    dWx = x.t().mm(dtanh)        # (D, H)
    dWh = prev_h.t().mm(dtanh)   # (H, H)
    db = dtanh.sum(dim=0)        # (H,)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for t in range(T):
        # استخراج ورودی در گام زمانی t
        xt = x[:, t, :] 
        # اجرای یک گام به جلو
        next_h, step_cache = rnn_step_forward(xt, prev_h, Wx, Wh, b)
        # ذخیره خروجی
        h[:, t, :] = next_h
        # به‌روزرسانی وضعیت برای گام بعدی
        prev_h = next_h
        # ذخیره کش هر مرحله
...
def rnn_backward(dh, cache):
    ...
    # مقداردهی اولیه گرادیان‌ها با صفر
    ...
    # حرکت رو به عقب در زمان (از T-1 تا 0)
    for t in reversed(range(T)):
        # جمع گرادیان فعلی با گرادیانی که از گام زمانی بعدی آمده
        dnext_h_t = dh[:, t, :] + dprev_h_t
        
        # محاسبه گرادیان یک گام
        dxt, dprev_h_t, dWxt, dWht, dbt = rnn_step_backward(dnext_h_t, cache[t])
...
def word_embedding_forward(x, W):
    # x: (N, T) ایندکس کلمات
    # W: (V, D) وزن‌های امبدینگ
    
    # استخراج بردارها بر اساس ایندکس‌ها
    out = W[x] 
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # x: امتیازها (N, T, V) - y: برچسب‌های واقعی (N, T) - mask: (N, T)
    N, T, V = x.shape
    
    # تغییر شکل برای محاسبه راحت‌تر
    x_flat = x.reshape(N * T, V)
    y_flat = y.reshape(N * T)
    mask_flat = mask.reshape(N * T)
    
    # محاسبه Softmax Loss برای تمام کلمات
    loss = F.cross_entropy(x_flat, y_flat, reduction='none')
    
    # اعمال ماسک: فقط خطای کلمات واقعی را نگه می‌داریم
    loss = loss * mask_flat
    
    # میانگین‌گیری بر اساس تعداد کل داده‌ها
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        h0 = self.feature_to_h0(features)

        # ۲. جدا کردن کلمات ورودی (همه به جز کلمه آخر)
        captions_in = captions[:, :-1]

        # ۳. تبدیل کلمات به بردار (Word Embedding)
        embeddings, _ = word_embedding_forward(captions_in, self.W_embed)

        # ۴. اجرای شبکه RNN (یا LSTM)
        if self.cell_type == 'rnn':
            h, _ = rnn_forward(embeddings, h0, self.Wx, self.Wh, self.b)
        elif self.cell_type == 'lstm':
            h, _ = lstm_forward(embeddings, h0, self.Wx, self.Wh, self.b)

        # ۵. تبدیل خروجی‌های RNN به امتیاز کلمات (Affine layer)
        scores = h.matmul(self.W_vocab) + self.b_vocab
